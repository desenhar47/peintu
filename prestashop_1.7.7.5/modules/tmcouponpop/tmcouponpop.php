...
include_once _PS_MODULE_DIR_.'tmcouponpop/classes/TmCouponPopClass.php';
include_once _PS_MODULE_DIR_.'tmcouponpop/sql/SampleDataCounpon.php';
class TmCouponPop extends Module
{
	...
	{
		$this->name = 'tmcouponpop';
		$this->tab = 'front_office_features';
		$this->version = '1.0.0';
		$this->author = 'templatemela';
		$this->secure_key = Tools::encrypt('templatemela'.$this->name);
		$this->bootstrap = true;
		parent::__construct();
		$this->displayName = $this->l('TM CouponPop Module');
		$this->description = $this->l('TM CouponPop Module');
	...
	{
		Configuration::updateValue('TM_VOUCHER_CODE','SAVE20');
		if (!parent::install() || !$this->registerHook('displayHeader') || !$this->registerHook('displayFooter')) return false;
		if (!Configuration::updateGlobalValue('MOD_TM_COUPONPOP', '1')) return false;
	...
	{
		Configuration::deleteByName('TM_VOUCHER_CODE');
		
		include(dirname(__FILE__).'/sql/uninstall.php');
		if (!parent::uninstall()) return false;
		if (!Configuration::deleteByName('MOD_TM_COUPONPOP')) return false;
		return true;
	}
	...
	private function _displayContact()
{
		...
		<br/>
	 	<fieldset>
			...
			For customizations or assistance, please contact: <strong><a   target="_blank" href="https://templatemela.com/contact-us">https://templatemela.com/contact-us</a></strong>
			<br>
			<a href="https://templatemela.com/" alt="templatemela" title="templatemela" target="_blank">https://templatemela.com/</a>
		</fieldset>';
		return $this->html;
}
	public function _displayAdvertising()
	{
		...
		<br/>
	 	<fieldset>
			<legend><img src="'.$this->_path.'views/img/help.png" alt="" title="" /> '.$this->l('Help').'</legend>		
			For customizations or assistance, please contact: <strong><a   target="_blank" href="https://templatemela.com/">https://templatemela.com/contact-us</a></strong>
			<br>
			
		</fieldset>
		<br/>
		<fieldset>
			<legend><img src="'.$this->_path.'views/img/more.jpg" alt="" title="" /> '.$this->l('More Themes & Modules').'</legend>	
			<iframe src="https://templatemela.com/advertising/prestashop_advertising.html" width="100%" height="420px;" border="0" style="border:none;"></iframe>
			</fieldset>';
		return $this->html;
	}
	...
	{		
		if (Tools::isSubmit('saveCoupon'))
		{
			...
			Configuration::updateValue('TM_VOUCHER_CODE', Tools::getValue('tm_voucher_code'));
			$coupon = new TmCouponPopClass(Tools::getValue('id_tmcouponpop'));
	...
	public function initForm()
	{
		...
		$id_tmcouponpop = 1;
		if ($id_tmcouponpop)
			$coupon = new TmCouponPopClass((int)$id_tmcouponpop);
		else
			$coupon = new TmCouponPopClass();
		...
		$helper->module = $this;
		$helper->name_controller = 'tmcouponpop';
		...
		$helper->fields_value['tm_voucher_code'] = Configuration::get('TM_VOUCHER_CODE');
		...
		if ($id_tmcouponpop)
		{
			$this->fields_form[0]['form']['input'][] = array('type' => 'hidden', 'name' => 'id_tmcouponpop');
			$helper->fields_value['id_tmcouponpop'] = (int)Tools::getValue('id_tmcouponpop', $coupon->id_tmcouponpop);	
		}
	...
	{
		$this->context->controller->addCSS(($this->_path).'views/css/tmcouponpop.css');
		$this->context->controller->addJS(($this->_path).'views/js/tmcouponpop.js');
	}
		...
		else if ($action == '0')
		{
			...
			if (!$this->isRegistered($register_status))
			{
				...
				else
				{
					...
					//if ($code = Configuration::get('TM_VOUCHER_CODE'))
	...
	public function hookdisplayFooter()
	{
		...
		if ($notshow >= 0)
		{
				...
				$sql = 'SELECT cpl.*, cps.*
						FROM `'._DB_PREFIX_.'tmcouponpop` cp
						LEFT JOIN `'._DB_PREFIX_.'tmcouponpop_shop` cps ON (cp.`id_tmcouponpop` = cps.`id_tmcouponpop`)
						LEFT JOIN `'._DB_PREFIX_.'tmcouponpop_lang` cpl ON (cps.`id_tmcouponpop` = cpl.`id_tmcouponpop`)	
						WHERE cpl.`id_shop` = '.$id_shop.' AND cpl.`id_lang` = '.$id_lang.' AND cp.`id_tmcouponpop` = 1';
				...
				if ($item)
					...
					$item = array();
				if (Tools::usingSecureMode())
					$tm_coupon_url = _PS_BASE_URL_SSL_.__PS_BASE_URI__.'modules/'.$this->name;
				else
					$tm_coupon_url = _PS_BASE_URL_.__PS_BASE_URI__.'modules/'.$this->name;
			...
			return $this->display(__FILE__, 'tmcouponpop.tpl');
		}
		else
			return false;
	}
	...
	{
		parent::_clearCache('tmcouponpop.tpl');
	}
	...
	{
		$verif_url = Context::getContext()->link->getModuleLink('tmcouponpop', 'verification', array('token' => $token,));
	...
	public function confirmEmail($token)
	{
		...
		if ($discount = Configuration::get('TM_VOUCHER_CODE'))

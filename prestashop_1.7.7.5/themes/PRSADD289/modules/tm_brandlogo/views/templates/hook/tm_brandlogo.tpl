...
<section class="brands">
	...
	<h2 class="h1 products-section-title text-uppercase">
		{if $display_link_brand}<a href="{$page_link}" title="{l s='Brands' d='Shop.Theme.Global'}">{/if}
			{l s='our happy clients' d='Shop.Theme.Global'}
  ...
  <div id="spe_res">
	<div class="products">
			...
			{foreach from=$brands item=brand name=brand_list}
				<li class="{if $slider == 1 && $brandCount >= $sliderFor}item{else}product_item col-xs-12 col-sm-4 col-md-3{/if}">
					...
					<a href="{$link->getmanufacturerLink($brand['id_manufacturer'], $brand['link_rewrite'])}" title="{$brand.name}">
						<img class="lazyload" data-src="{$link->getManufacturerImageLink($brand['id_manufacturer'])}" alt="{$brand.name}" />
					</a>
					</div>
					{if $brandname}
		...
		{else}
			<p>{l s='No brand' d='Shop.Theme.Global'}</p>
		{/if}
	</div>
</div>
</div>
